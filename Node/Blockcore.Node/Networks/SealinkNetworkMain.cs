...
using SealinkNetwork.Networks.Policies;
using SealinkNetwork.Networks.Rules;
using System.Linq;
using System.Net;
...
using SealinkNetwork.Networks.Deployments;
...
{
    public class SealinkNetworkMain : Network
   {
      public SealinkNetworkMain()
      {
         CoinSetup setup = SealinkNetworkSetup.Instance.Setup;
         NetworkSetup network = SealinkNetworkSetup.Instance.Main;

         NetworkType = NetworkType.Mainnet;
         DefaultConfigFilename = setup.ConfigFileName; // The default name used for the SealinkNetwork configuration file.
         ...
         var bip9Deployments = new SealinkNetworkBIP9Deployments()
         {
            [SealinkNetworkBIP9Deployments.ColdStaking] = new BIP9DeploymentsParameters("ColdStaking", 2,
         ...
         Consensus = new Blockcore.Consensus.Consensus(
             ...
             coinbaseMaturity: 10,//pos mature
             premineHeight: 2,
         ...
         Consensus.PosEmptyCoinbase = SealinkNetworkSetup.Instance.IsPoSv3();
         Consensus.PosUseTimeFieldInKernalHash = SealinkNetworkSetup.Instance.IsPoSv3();
         ...
         SeedNodes = network.Nodes.Select(node => new Blockcore.NBitcoin.Protocol.NetworkAddress(IPAddress.Parse(node), network.DefaultPort)).ToList();

         StandardScriptsRegistry = new SealinkNetworkStandardScriptsRegistry();
      ...
      protected void RegisterRules(IConsensus consensus)
      {
         consensus.ConsensusRules
             ...
             .Register<SealinkNetworkHeaderVersionRule>()
